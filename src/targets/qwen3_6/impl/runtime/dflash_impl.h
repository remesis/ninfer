...
auto dflash_decode_batch_body(DFlashBatchContext& state, std::int32_t batch_size, std::uint32_t k,
    ...
    return [&state, batch_size, k, envelopes, target_envelope] {
        ...
        const std::size_t ingress_bytes =
            state.ngram ? sizeof(qwen3_6::DFlashDecodeIngress)
                        : offsetof(qwen3_6::DFlashDecodeIngress, ngram_tokens);
        ...
        Tensor compact_features =
            state.execution.work.alloc(DType::BF16, {Variant::DFlashConfig::feature_rows,
        ...
        if (state.ngram) {
            ...
            CUDA_CHECK(cudaMemcpyAsync(
                ...
                ingress + offsetof(qwen3_6::DFlashDecodeIngress, ngram_tokens), k * sizeof(TokenId),
            ...
            if constexpr (Variant::DFlashConfig::coherent_selector) {
                ...
                CUDA_CHECK(cudaMemcpyAsync(
                    ...
                    ingress + offsetof(qwen3_6::DFlashDecodeIngress, ngram_candidates),
                    k * 16 * sizeof(TokenId), cudaMemcpyDeviceToDevice,
                    state.execution.device.stream));
                CUDA_CHECK(
                    cudaMemcpyAsync(frame.proposal_q.data,
                                    ingress + offsetof(qwen3_6::DFlashDecodeIngress, ngram_q),
                                    k * 16 * sizeof(float), cudaMemcpyDeviceToDevice,
                                    state.execution.device.stream));
            }
        } else {
            ...
            propose_batch_impl<Variant>(state, proposal_frame, batch_size, proposal_k, envelopes);
        }
