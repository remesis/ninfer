...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // Preserve the original fresh-prefill diagnostic separately from the exact
        // retained-state control; reaching this check does not waive its discrepancy.
        if (!fresh_equivalent) {
            std::cerr << "fresh-prefill equivalence remains unresolved" << std::endl;
            return 2;
        }
