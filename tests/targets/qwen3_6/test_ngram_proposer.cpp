#include "targets/qwen3_6/impl/runtime/ngram_proposer.h"
...
using ninfer::targets::qwen3_6::detail::NgramProposer;
using ninfer::targets::qwen3_6::frontend_internal::ngram_numbered_sources;
...
int main() {
    try {
        ...
        // Two near-capacity corpora pressure both raw-prompt and derived-source retention.
        std::vector<int> old_source(2048), pressure(240000);
        std::mt19937 pressure_random(91026);
        for (auto& token : old_source) { token = 100 + pressure_random() % 150000; }
        for (auto& token : pressure) { token = 100 + pressure_random() % 150000; }
        for (bool derived : {false, true}) {
            ...
            for (std::size_t end = 64; end + 15 <= old_source.size(); ++end) {
                ...
                require(
                    std::equal(draft.tokens.begin(), draft.tokens.end(), old_source.begin() + end),
